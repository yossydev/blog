...
まずこのブログではさらっと書かれていますが、[Announcing the Web Platform Dashboard](https://web.dev/blog/web-platform-dashboard)で、[Web Platform Dashboard](https://webstatus.dev/)を作ったことが書いてあります。
これは新しいJavaScriptやCSSが**どのブラウザがどれくらい対応しているのか**がわかりやすくまとまっているサイトになります。そしてこれは[GoogleChrome/webstatus.dev](https://github.com/GoogleChrome/webstatus.dev)で、OSSになっています。

I/Oでは12個の機能を紹介したそうです。それぞれ以下です。
...
- CSS grid layout subgrid
  - gridレイアウト内でさらに細かな指定ができるようになった的な認識です。
...
- Responsive video
  ...
  - ```html
    ...
    ```
- The inert attribute
  - `inert`を指定することで、以下のことを行うそうです。（[MDN](https://developer.mozilla.org/ja/docs/Web/HTML/Global_attributes/inert)から参照）
...
- `:user-valid` and `:user-invalid`
  ...
  - [MDN]https://developer.mozilla.org/ja/docs/Web/CSS/:user-invalidのサンプルがすごいわかりやすい。とても使えそう。 
- Compression Streams
